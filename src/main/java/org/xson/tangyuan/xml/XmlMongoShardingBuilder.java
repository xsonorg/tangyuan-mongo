...
public class XmlMongoShardingBuilder {
	...
	private Map<String, MongoDataSourceVo>	dataSourceVoMap			= null;
	...
	public XmlMongoShardingBuilder(InputStream inputStream, Map<String, MongoDataSourceVo> dataSourceVoMap) {
	...
	private void buildShardingClassNodes(List<XmlNodeWrapper> contexts) throws Exception {
		...
		for (XmlNodeWrapper xNode : contexts) {
			...
			if (null != mappingClassMap.get(id)) {
				throw new XmlParseException("Duplicate shardingClass id: " + id);
			}
	...
	private void buildShardingTableNodes(List<XmlNodeWrapper> contexts) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (null == dataSource || 0 == dataSource.length()) {
				throw new XmlParseException("Invalid ShardingTable dataSource:" + dataSource);
			}
			MongoDataSourceVo dsVo = dataSourceVoMap.get(dataSource);
			if (null == dsVo) {
				throw new XmlParseException("Nonexistent dataSource: " + dataSource);
			}
